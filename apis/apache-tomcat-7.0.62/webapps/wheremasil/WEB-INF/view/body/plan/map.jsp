...
<script type="text/javascript">
	...
	$(function() {
		...
		$(".left_nav").css("height", "800px");
		$(".right_nav").css("height", "800px");
		        		...
		        		for ( var i = 0; i < data.length; i++ ) {
		    		        ...
		    		        (function(marker, title, addr, img, id) {
		    		            ...
		    		            daum.maps.event.addListener(marker, 'mouseup', function() {
		    		    			bounds = new daum.maps.LatLngBounds(marker.getPosition(), marker.getPosition());
		...
		function displayPlaces(places) {
		    ...
		    for ( var i=0; i<places.length; i++ ) {
		    	var dupFlg = false;
				var regExp = /[\s*\{\}\[\]\/?.,;:|\)*~`!^\-_+<>@\#$%&\\\=\(\'\"]/gi;
				var fixedTitle = places[i].title.replace(regExp, '');
		    	for (var idx = 0; idx < titles.length; idx++) {
					if (fixedTitle == titles[idx]) {
						dupFlg = !dupFlg;
						break;
		    	...
		    	if (dupFlg) {
		    		for (var idx = 0; idx < areaMarkers.length; idx++) {
		    			if (areaMarkers[idx].getTitle() == places[i].title) {
		    				marker = areaMarkers[idx];
		    			}
		    		}
		    	}
